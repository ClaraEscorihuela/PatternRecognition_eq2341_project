...
def GetMusicFeatures(signal, fs, winlength=0.06):
    ...
    winlength = max(
        [winlength + (winlength % 2), 2 * minlag]
    )  # Make windows sufficiently long and an even sample number
    ...
    for n in range(nsteps):
        ...
        frIsequence[:, n] = [
            fs / pprd,
            maxcorr,
            np.linalg.norm(window / np.sqrt(len(window))),
        ]
