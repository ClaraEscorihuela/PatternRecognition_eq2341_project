...
class DiscreteD:
    ...
    def __init__(self, x):
        self.pseudoCount = 0
        self.probMass = x / np.sum(x)  # X are the observations?
    ...
    def rand(self, nData):
        ...
        return random.choices(self.probMass, k=nData)
    ...
    def init(self, x):
        ...
        x = np.round(x)
        maxObs = int(np.max(x))  # busco la observació maxima
        # collect observation frequencies
        fObs = np.zeros(maxObs)  # observation frequencies: creo  vector amb tants zeros com obs max

        for i in range(maxObs):
            fObs[i] = 1 + np.sum(x == i)  # conto num de observacions i sumo 1 per evitar q hi hagi 0

        self.probMass = fObs / np.sum(fObs)  # faig poba dividint entre el max d'obs
