...
class HMM:
    ...
    """
    def __init__(self, mc: MarkovChain.MarkovChain, distributions: List[Distributions]):

        self.stateGen: MarkovChain.MarkovChain = mc
        self.outputDistr: List[Distributions] = distributions

        self.nStates = mc.nStates
        self.dataSize = distributions[0].dataSize


    def rand(self, nSamples: int) -> Tuple[np.array, np.array]:
        """
        ...
        s_seq = self.stateGen.rand(tmax=nSamples)
        x_seq = []
        n_dims = self.outputDistr[0].nDims

        for s in s_seq:
            distribution = self.outputDistr[s]
            x_seq.append(distribution.rand(nData=n_dims)[0])

        x_seq = np.array(x_seq).reshape(-1, n_dims).T
        return x_seq, np.array(s_seq)
